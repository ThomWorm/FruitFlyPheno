...
def fflies_model_run(
    start_date,
    degree_day_data,
    dd_threshold,
    window=None,
    use_current_years_weather=True,
    generations=3,
):
    # S
    ...
    for year in range(2000, 2020):
        ...
        temporary_degree_day_data = degree_day_data.sel(
            t=slice(start_day_loop, None)
        ).copy()
        ...
        if number_days_recent_data > 0:
            temporary_degree_day_data.loc[
                {"t": slice(start_day_loop, finish_day_loop)}
            ] = degree_day_data_current.values

        f3_maturation = calculate_maturation_date(
            temporary_degree_day_data, start_day_loop, dd_threshold, generations
        )
        ...
        f3_maturation["timedelta"] = (
            (f3_maturation["gen_comp_date"].data - start_day_loop)
            / (24 * 60 * 60 * 1e9)
        ).astype(int)
...
def testing_load_data(
    ...
    if "prism_year_stack" not in locals() and "degree_day_data" not in locals():
        ...
        if lat is not None and lon is not None:
            prism_year_stack, na_mask = subset_dataset_by_coords(
                prism_year_stack, lat, lon, window
            )
        elif (
            lat_min is not None
            and lat_max is not None
            ...
            prism_year_stack, na_mask = subset_dataset_by_coords(
                prism_year_stack, lat, lon, lat_min, lat_max, lon_min, lon_max
            )
